...
const USDCSponsoredBatchedTransaction: React.FC<{
  ...
  const createSession = async () => {
    ...
    try {
      ...
      const biconomyPaymaster =
        smartAccount.paymaster as IHybridPaymaster<SponsorUserOperationDto>;
      const fetchFeequote = async (userOps) => {
        const feeQuotesResponse =
          await biconomyPaymaster.getPaymasterFeeQuotesOrData(userOps, {
      ...
      const { feeQuotes, tokenPaymasterAddress } = await fetchFeequote(
        partialUserOp
      );
      console.log("master-deb 1 ", feeQuotes, tokenPaymasterAddress);

      const USDCFeeQuotes = feeQuotes?.[2];
      if (!USDCFeeQuotes) {
        console.log("master-deb not found");
      }
      ...
      console.log("master-deb 2 usdc build ", userOp);
      ...
      console.log("master-deb 3 paymaster ready ", userOp);
...
export default USDCSponsoredBatchedTransaction;
